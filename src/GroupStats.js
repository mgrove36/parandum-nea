...
export default withRouter(
  class GroupStats extends Component {
          ...
          await this.state.db
            ...
            .then((querySnapshot) => {
              ...
              querySnapshot.docs.map((doc, index, array) => {
                if (
                  doc.data().setIds.some((item) => groupSetIds.includes(item))
                ) {
                  if (
                    index === 0 ||
                    doc.data().term !== array[index - 1].data().term ||
                    doc.data().definition !== array[index - 1].data().definition
                  ) {
                  ...
                  doc
                    .data()
                    .setIds.map(
                      (setId) => (newState.setsWithHistory[setId] = true)
                    );
              ...
              newState.incorrectAnswers = incorrectAnswers.sort(
                (a, b) => b.count + b.switchedCount - a.count - a.switchedCount
              );
    ...
    handleSetSelectionChange = (selectedSet = this.state.selectedSet) => {
      ...
      const filteredIncorrectAnswers = (
        selectedSet.value === "all_sets"
          ? JSON.parse(JSON.stringify(this.state.incorrectAnswers))
          : JSON.parse(JSON.stringify(this.state.incorrectAnswers)).filter(
              (vocabItem) => vocabItem.setIds.includes(selectedSet.value)
            )
      ).map((vocabItem) => {
        ...
        if (selectedSet.value === "all_sets") {
          if (this.state.includeCompoundTests) {
            ...
            newVocabItem.answers = vocabItem.answers.filter(
              (answer) => answer.setIds.length === 1
            );
          }
        } else {
          newVocabItem.answers = vocabItem.answers.filter(
        ...
        newVocabItem.switchedCount = newVocabItem.answers.filter(
          (answer) => answer.switchLanguage === true
        ).length;
        newVocabItem.count =
          newVocabItem.answers.length - newVocabItem.switchedCount;
              ...
              <div className="history-sections">
                <Select
                  ...
                  options={[
                    ...
                    Object.keys(this.state.sets)
                      .filter(
                        (setId) => this.state.setsWithHistory[setId] === true
                      )
                      .sort((a, b) => {
                        if (
                          this.state.sets[a].title < this.state.sets[b].title
                        ) {
                          return -1;
                        }
                        if (
                ...
                <div className="historical-user-stats-container">
                  ...
                  {this.state.filteredIncorrectAnswers.length > 0 && (
                    <div className="stat-row stat-row--inline">
                      ...
                      <p>meaning</p>
                      <h1>
                        {this.state.filteredIncorrectAnswers[0].definition}
                      </h1>
                ...
                <div className="mistakes-history-container">
                  {this.state.filteredIncorrectAnswers
                    .filter(
                      (vocabItem) =>
                        vocabItem.answers && vocabItem.answers.length > 0
                    )
                    .map((vocabItem, index) => {
                      ...
                      vocabItem.answers.map((answerItem) => {
                        if (answerItem.switchLanguage) {
                          if (
                            switchedAnswersDict.hasOwnProperty(
                              answerItem.answer
                            )
                          ) {
                          ...
                          if (
                            notSwitchedAnswersDict.hasOwnProperty(
                              answerItem.answer
                            )
                          ) {
                      ...
                      const notSwitchedAnswers = Object.keys(
                        notSwitchedAnswersDict
                      )
                            ...
                            {vocabItem.switchedCount > 0 ? (
                              <Collapsible
                                ...
                                trigger={
                                  <>
                                    <b>
                                      {vocabItem.switchedCount} mistake
                                      {vocabItem.switchedCount !== 1 && "s"}
                                    </b>
