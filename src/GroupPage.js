  ...
  class GroupPage extends Component {
    constructor(props) {
      ...
      this.state = {
        ...
        functions: {
          removeSetFromGroup:
            props.functions.httpsCallable("removeSetFromGroup"),
    ...
    async componentDidMount() {
      ...
      newState.memberCount =
        ...
        (Object.keys(completedPromises[1].users).includes(this.state.user.uid)
          ? 0
          : 1);
      newState.joinCode =
        completedPromises[0].role === "owner"
          ? completedPromises[1].join_code
          : "";
        ...
        if (role === "remove") {
          ...
          this.state.db
            ...
            .then(() => {
              ...
              let userData;
              if (this.state.editingUser.role === "owner") {
                userData = groupUsers.owners.splice(
                  this.state.editingUser.index,
                  1
                )[0];
              } else if (this.state.editingUser.role === "contributor") {
                userData = groupUsers.contributors.splice(
                  this.state.editingUser.index,
                  1
                )[0];
              } else {
                userData = groupUsers.members.splice(
                  this.state.editingUser.index,
                  1
                )[0];
              }
                  ...
                  {Object.keys(this.state.sets).length > 0 ? (
                    <div className="group-links-container">
                      {Object.keys(this.state.sets)
                        .sort((a, b) => {
                          if (
                            this.state.sets[a].displayName <
                            this.state.sets[b].displayName
                          ) {
                            return -1;
                          }
                          if (
                  ...
                  <>
                    <div>
                      ...
                      {this.state.groupUsers &&
                        this.state.groupUsers.owners.length > 0 && (
                          <>
                            <h3 className="group-role-header">Owners</h3>
                            <div className="group-links-container">
                              {this.state.groupUsers.owners.map(
                                (user, index) => (
                                  <p
                                    key={user.uid}
                      ...
                      {this.state.groupUsers &&
                        this.state.groupUsers.contributors.length > 0 && (
                          <>
                            <h3 className="group-role-header">Contributors</h3>
                            <div className="group-links-container">
                              {this.state.groupUsers.contributors.map(
                                (user, index) => (
                                  <p
                                    key={user.uid}
                                    className={`group-set-link ${
                      ...
                      {this.state.groupUsers &&
                        this.state.groupUsers.members.length > 0 && (
                          <>
                            <h3 className="group-role-header">Members</h3>
                            <div className="group-links-container">
                              {this.state.groupUsers.members.map(
                                (user, index) => (
                                  <p
                                    key={user.uid}
                                    className={`group-set-link ${
                    ...
                    {this.state.editingUser && (
                      <>
                        <div
                          className="overlay"
                          onClick={this.hideEditUserRole}
                        ></div>
                        <div className="overlay-content options-list-overlay-content">
                          {["Owner", "Contributor", "Member", "Remove"].map(
                            (role) => (
                              <h3
                                key={role}
                                onClick={() =>
