        ...
        .then(async (querySnapshot) => {
          ...
          querySnapshot.docs.map((doc, index, array) => {
            ...
            doc
              ...
              .setIds.map(
                (setId, setIndex) =>
                  (newState.setsWithHistory[setId] =
                    doc.data().set_titles[setIndex])
              );
    ...
    this.setState({
      ...
      totalTests:
        selectedSet.value === "all_sets"
          ...
          : this.state.progressHistory.filter(
              (progressItem) =>
                this.arraysHaveSameMembers(progressItem.setIds, [
                  selectedSet.value,
                ]) ||
                (this.state.includeCompoundTests &&
          ...
          <div className="history-sections">
            <Select
              ...
              options={[
                ...
                Object.keys(this.state.setsWithHistory)
                  .sort((a, b) => {
                    if (
                      this.state.setsWithHistory[a] <
                      this.state.setsWithHistory[b]
                    ) {
                      return -1;
                    }
                    if (
            ...
            <div className="historical-user-stats-container">
              ...
              {this.state.totalTests > 0 && (
                <div className="stat-row stat-row--inline">
                  ...
                  <h1>
                    {(
                      this.state.totalIncorrect / this.state.totalTests
                    ).toFixed(2)}
                  </h1>
                  <p>per test</p>
                </div>
                ...
                .map((vocabItem, index) => {
                  ...
                  vocabItem.answers.map((answerItem) => {
                    if (answerItem.switchLanguage) {
                      if (
                        switchedAnswersDict.hasOwnProperty(answerItem.answer)
                      ) {
                      ...
                      if (
                        notSwitchedAnswersDict.hasOwnProperty(answerItem.answer)
                      ) {
