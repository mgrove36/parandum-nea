  ...
  class LoggedInHome extends React.Component {
    ...
    componentDidMount() {
      ...
      const userSetsQuery = userSetsRef.get().then((userSetsQuerySnapshot) => {
        ...
        userSetsQuerySnapshot.docs.map(
          (doc) => (newState.selections[doc.id] = false)
        );
      });
      const publicSetsQuery = publicSetsRef
        .get()
        .then((publicSetsQuerySnapshot) => {
          newState.publicSets = publicSetsQuerySnapshot.docs;

          publicSetsQuerySnapshot.docs.map(
            (doc) => (newState.selections[doc.id] = false)
          );
        });
      const userGroupsQuery = userGroupsRef
        .get()
        .then(async (userGroupsQuerySnapshot) => {
          newState.user.groups = [];
          var userGroupSets = [];

          return Promise.all(
            userGroupsQuerySnapshot.docs.map((group) => {
              const groupData = groupRef
                .doc(group.id)
                .get()
                .catch((error) => {
              ...
              return userGroupSetsRef
                ...
                .get()
                .then(async (userGroupSetsQuerySnapshot) => {
                  groupData.then((result) => {
                    if (
                      typeof result !== "undefined" &&
                      typeof result.data === "function" &&
                      userGroupSetsQuerySnapshot.docs.length > 0
                    ) {
                      userGroupSets.push({
                        group: result,
                      ...
                      userGroupSetsQuerySnapshot.docs.map(
                        (doc) => (newState.selections[doc.id] = false)
                      );
                    }
                  });
      ...
      const progressQuery = progressRef.get().then((progressQuerySnapshot) => {
        progressQuerySnapshot.docs.map((doc) => {
          ...
          return newState.progressHistoryIncomplete.push({
            ...
            grade: (data.progress > 0
              ? (data.correct.length / data.progress) * 100
              : 0
            ).toFixed(2),
            mode: data.mode,
    ...
    startTest = (mode) => {
      if (this.state.canStartTest) {
        const selections = Object.keys(this.state.selections).filter(
          (x) => this.state.selections[x]
        );
    ...
    deleteProgress = (progressId) => {
      if (this.state.pendingDeletions[progressId] !== true) {
        ...
        this.state.db
          ...
          .delete()
          .then(() => {
            const progressIndex = this.state.progressHistoryIncomplete
              .map((obj) => obj.id)
              .indexOf(progressId);
            let newState = {
    ...
    showIndividualTestPrompt = async (mode) => {
      if (!this.state.loading) {
        if (mode === "classic") {
          ...
          const setIds = Object.keys(this.state.selections).filter(
            (x) => this.state.selections[x]
          );
          ...
          <main>
            <div className="page-header page-header--home">
              ...
              <div className="button-container buttons--desktop">
                ...
                >
                  Test (
                  {
                    Object.values(this.state.selections).filter(
                      (x) => x === true
                    ).length
                  }
                  )
                </Button>
                <LinkButton to="/groups">Groups</LinkButton>
                {(!this.props.page.loaded() ||
                  (this.state.userSets && this.state.userSets.length > 0)) && (
                  <LinkButton to="/my-sets">My Sets</LinkButton>
                )}
                <LinkButton
                  id="create-set-button-desktop"
                  to="/create-set"
                  icon={<AddRoundedIcon />}
            ...
            <div className="page-header page-header--left buttons--mobile">
              ...
              >
                Test (
                {
                  Object.values(this.state.selections).filter((x) => x === true)
                    .length
                }
                )
              </Button>
                ...
                <div className="progress-history-container progress-history-container--incomplete">
                  ...
                  {this.state.progressHistoryIncomplete.map((progressItem) => (
                    <div key={progressItem.id}>
                      <Link to={`/progress/${progressItem.id}`}>
                      ...
                      <p>
                        <Button
                          ...
                          disabled={
                            this.state.pendingDeletions[progressItem.id] ===
                            true
                          }
                          loading={
                            this.state.pendingDeletions[progressItem.id] ===
            ...
            {this.state.userSets &&
              this.state.userSets.length === 0 &&
              this.state.userGroupSets &&
              this.state.userGroupSets.length === 0 &&
              this.state.progressHistoryIncomplete.length === 0 && (
                <>
                  <Xarrow
                    start="page-intro"
            ...
            <div className="form set-list">
              {this.state.userSets && this.state.userSets.length > 0 && (
                <div className="checkbox-list-container">
                  <h3>
                    <PersonRoundedIcon /> Personal Sets
                  </h3>
