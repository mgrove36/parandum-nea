  ...
  class SetPage extends React.Component {
    ...
    showAddSetToGroup = async () => {
      ...
      await this.state.db
        ...
        .then((querySnapshot) => {
          return Promise.all(
            querySnapshot.docs.map((userGroupDoc) => {
              if (!this.state.currentSetGroups.includes(userGroupDoc.id))
                return this.state.db
                  ...
                  .get()
                  .then((groupDoc) => {
                    newState.groups[userGroupDoc.id] =
    ...
    stopAddSetToGroupLoading = (groupId, addedSetToGroup = false) => {
      ...
      if (addedSetToGroup)
        newState.currentSetGroups = this.state.currentSetGroups.concat(groupId);
      this.setState(newState);
    };
    ...
    showIndividualTestPrompt = async (mode) => {
      if (!this.state.loading) {
        if (mode === "classic") {
          ...
          const totalTestQuestions = await this.state.db
            .collection("sets")
            .doc(this.props.match.params.setId)
            .collection("vocab")
            .get()
              ...
              <main>
                <div className="page-header">
                  ...
                  <div className="button-container">
                    ...
                    {this.state.set.owner === this.state.user.uid &&
                      this.state.currentSetGroups.length === 0 && (
                        <Button
                  ...
                  <div className="overlay-content set-page-group-overlay-content">
                    {Object.keys(this.state.groups).length < 1 ? (
                      <>
                        ...
                        <p>
                          To add sets to a group, you must be an owner or
                          collaborator.
                        </p>
                      </>
                    ) : (
