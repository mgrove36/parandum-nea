...
export default withRouter(
  class Progress extends React.Component {
    ...
    constructor(props) {
      ...
      this.state = {
        ...
        functions: {
          ...
          createProgressWithIncorrect: props.functions.httpsCallable(
            "createProgressWithIncorrect"
          ),
        },
        navbarItems: [
          {
    ...
    async componentDidMount() {
      this.unlisten = this.props.history.listen((location, action) => {
        if (location.pathname.startsWith("/progress/"))
          this.setState(this.changeableStateItems, () =>
      ...
      let [newState, setDone, incorrectAnswers, duration, setIds] =
        await progressRef
          .get()
          .then((doc) => {
            const data = doc.data();
        ...
        if (!setDone) {
          ...
          promises.push(
            this.state.db
              ...
              .get()
              .then((querySnapshot) => {
                newState.attemptNumber =
                  querySnapshot.docs
                    .map((doc) => doc.id)
                    .indexOf(this.props.match.params.progressId) + 1;
                if (querySnapshot.docs.length > 1)
                  newState.attemptHistory = querySnapshot.docs
                    .filter((doc) => doc.data().duration !== null)
                    .map((doc) => {
                      if (doc.id === this.props.match.params.progressId)
                        newState.startTime = doc.data().start_time;
                      return {
                        x: new Date(doc.data().start_time),
                        y:
                          (doc.data().correct.length /
          ...
          promises.push(
            this.state.db
              ...
              .then((completedProgressDoc) => {
                newState.averagePercentage = (
                  completedProgressDoc.data().total_percentage /
                  completedProgressDoc.data().attempts
                ).toFixed(2);
              })
              .catch((error) => {
                  ...
                  if (newState.incorrectAnswers[vocabId]) {
                    ...
                    return Promise.all([
                      progressRef
                        ...
                        .get()
                        .then((termDoc) => {
                          newState.switchLanguage
                            ? (newState.incorrectAnswers[vocabId].answer =
                                termDoc.data().item.split("/"))
                            : (newState.incorrectAnswers[vocabId].prompt =
                                termDoc.data().item);
                        }),
                      progressRef
                        .collection("definitions")
                        ...
                        .then((definitionDoc) => {
                          newState.switchLanguage
                            ? (newState.incorrectAnswers[vocabId].prompt =
                                definitionDoc.data().item)
                            : (newState.incorrectAnswers[vocabId].answer =
                                definitionDoc.data().item.split("/"));
                        }),
                    ]);
                  }
                })
    ...
    handleThemeInputChange = (newTheme) => {
      if (this.state.themeInput !== newTheme)
        this.setState({
          themeInput: newTheme,
        });
    };
    ...
    saveSettings = (globalChange) => {
      ...
      this.props.handleColoredEdgesChange(
        this.state.coloredEdgesInput,
        globalChange
      );
      this.hideSettings();
    };
    ...
    processAnswer = async () => {
      if (this.state.canProceed) {
        const cleansedCurrentCorrect = this.state.currentCorrect.map((item) =>
          this.cleanseVocabString(item)
        );

        this.startLoading();

        if (
          !cleansedCurrentCorrect.includes(
            this.cleanseVocabString(this.state.answerInput)
          )
        ) {
          this.state.functions
            ...
            .then(async (result) => {
              ...
              let newState = {
                ...
                nextSetOwner: data.nextPrompt
                  ? data.nextPrompt.set_owner
                  : null,
                progress: data.progress,
              ...
              if (data.correct) {
                ...
                if (!data.moreAnswers)
                  newState = this.showNextQuestion(newState, newState);
              } else {
              ...
              if (
                ...
                newState.incorrectAnswers = this.state.incorrectAnswers;
                newState.incorrectAnswers[data.currentVocabId].answer =
                  data.correctAnswers;
              }
              ...
              if (data.incorrectAnswers) {
                ...
                data.incorrectAnswers.map((vocabId) => {
                  if (
                    ...
                    unsavedAnswers[vocabId]
                      ? unsavedAnswers[vocabId]++
                      : (unsavedAnswers[vocabId] = 1);
                  }
                  return true;
                });
                ...
                promises.push(
                  Promise.all(
                    Object.keys(unsavedAnswers).map((vocabId) => {
    ...
    msToTime = (time) => {
      ...
      const seconds = Math.floor((time / 1000) % 60).toLocaleString(
        "en-GB",
        localeData
      );
      const minutes = Math.floor((time / 1000 / 60) % 60).toLocaleString(
        "en-GB",
        localeData
      );
      const hours = Math.floor(time / 1000 / 60 / 60).toLocaleString(
        "en-GB",
        localeData
      );
          ...
          .createProgress({
            ...
            limit:
              this.state.mode === "questions"
                ? this.state.progress - this.state.incorrect
                : this.state.mode === "lives"
                ? this.state.lives
                : 1,
          })
          .then((result) => {
    ...
    createTestWithIncorrect = () => {
      if (!this.state.loading) {
        this.state.functions
          .createProgressWithIncorrect(this.props.match.params.progressId)
          .then((result) => {
            const progressId = result.data;
            this.stopLoading();
            this.props.history.push("/progress/" + progressId);
          ...
          .catch((error) => {
            console.log(
              `Couldn't create test with incorrect answers: ${error}`
            );
            this.stopLoading();
          });
                ...
                !this.state.setComplete ? (
                  <main className="progress-container">
                    <div>
                      ...
                      <p className={!this.state.typo ? "hide" : ""}>
                        Are you sure?
                      </p>
                      <div className="correct-answers">
                        {this.state.currentCorrect &&
                        this.state.currentCorrect.length > 0 ? (
                          <>
                            <h2>
                  ...
                  <main>
                    ...
                    <div className="progress-stat-row-container">
                      <div className="stat-row stat-row--inline">
                        <p>You got</p>
                        <h1>{`${(
                          (this.state.correct / this.state.totalQuestions) *
                          100
                        ).toFixed(2)}%`}</h1>
                      </div>
                      <div className="stat-row stat-row--inline">
                    ...
                    <div className="progress-end-button-container">
                      <Button onClick={this.showTestRestart}>
                        Restart test
                      </Button>
                      {this.state.incorrect > 0 && (
                        <Button onClick={this.showIncorrectTestStart}>
                    ...
                    {this.state.incorrectAnswers &&
                      Object.keys(this.state.incorrectAnswers).length > 0 && (
                        <>
                          ...
                          <div className="progress-end-incorrect-answers">
                            ...
                            {Object.keys(this.state.incorrectAnswers)
                              .map((key) => [
                                key,
                                this.state.incorrectAnswers[key].count,
                              ])
                              .sort((a, b) => b[1] - a[1])
                              .map((item) => (
                                <div key={item[0]}>
                                  <p>
                                    {this.state.incorrectAnswers[item[0]].prompt
                                      ? this.state.incorrectAnswers[item[0]]
                                          .prompt
                                      : ""}
                                  </p>
                                  <p>
                                    {this.state.incorrectAnswers[item[0]].answer
                                      ? this.state.incorrectAnswers[
                                          item[0]
                                        ].answer.join("/")
                                      : ""}
                    ...
                    {Object.keys(this.state.attemptHistory).length > 1 && (
                      <>
                        ...
                        <LineChart
                          data={this.state.attemptHistory}
                          currentPointX={this.state.startTime}
                        />
                      </>
                    )}
                  </main>
                  ...
                  <main className="progress-container">
                    ...
                    <div>
                      ...
                      <form
                        className="answer-input-container answer-input-container--answer-entered"
                        onSubmit={(e) => e.preventDefault()}
                      >
                        <input
                          type="submit"
                        ...
                        <input
                          ...
                          name="answer_input"
                          className={`answer-input ${
                            this.state.currentAnswerStatus
                              ? "answer--correct"
                              : "answer--incorrect"
                          }`}
                          value={this.state.answerInput}
                ...
                {!this.state.setComplete && (
                  <ProgressStats
                    ...
                    progressDenominator={
                      this.state.mode === "lives"
                        ? this.state.startLives
                        : this.state.originalTotalQuestions
                    }
                    progress={this.state.progress}
                    grade={
                      this.state.correct + this.state.incorrect > 0
                        ? (this.state.correct /
                  ...
                  <>
                    ...
                    <div className="overlay-content progress-settings-overlay-content">
                      <SettingsContent
                        ...
                        handleColoredEdgesInputChange={
                          this.handleColoredEdgesInputChange
                        }
                      ...
                      <div className="settings-save-container">
                        <Button onClick={() => this.saveSettings(true)}>
                          Save
                        </Button>
                        <Button onClick={() => this.saveSettings(false)}>
