...
export default withRouter(
  class EditSet extends Component {
    ...
    componentDidMount() {
      ...
      const setRef = this.state.db.collection("sets").doc(setId);
      const setVocabRef = setRef.collection("vocab").orderBy("term");
        ...
        .then((setDoc) => {
          ...
          setVocabRef.get().then((querySnapshot) => {
            ...
            querySnapshot.docs.map((doc) => {
              ...
              if (
                this.cleanseVocabString(data.term) !== "" &&
            ...
            let newState = {
              ...
              canMakeSetNonPublic: !(
                setDoc.data().groups && setDoc.data().groups.length > 0
              ),
            };

            if (
              !(
                newState.inputs.title !== "" &&
                vocabPairsCount > 0 &&
                vocabPairsCount === this.state.inputContents.length
              )
            ) {
    ...
    componentWillUnmount = () => {
      window.removeEventListener(
        "beforeunload",
        this.alertLeavingWithoutSaving
      );
    ...
    handleSetDataChange = () => {
      const numberOfVocabPairs = this.state.inputContents
        .map(
          (contents) =>
      ...
      if (
        ...
        numberOfVocabPairs > 0 &&
        numberOfVocabPairs === this.state.inputContents.length
      ) {
        this.setState({
          canSaveSet: true,
        });
    ...
    onTermInputChange = (event) => {
      ...
      if (index >= this.state.inputContents.length && input !== "") {
        ...
        if (
          index === this.state.inputContents.length - 1 &&
          input === "" &&
          this.state.inputContents[index].definition === ""
        ) {
    ...
    onDefinitionInputChange = (event) => {
      ...
      if (index >= this.state.inputContents.length && input !== "") {
        ...
        if (
          index === this.state.inputContents.length - 1 &&
          input === "" &&
          this.state.inputContents[index].term === ""
        ) {
          inputContents.splice(-1);
        } else {
    ...
    onPublicSetInputChange = (event) => {
      if (this.state.canMakeSetNonPublic)
        this.setState(
          {
            inputs: {
              ...this.state.inputs,
    ...
    saveSet = async () => {
      if (this.state.canSaveSet) {
        ...
        if (
          this.state.inputContents.length <
          this.state.originalInputContents.length
        ) {
          let batchItems = this.state.inputContents.length % 248;
          for (
            let i = this.state.inputContents.length;
            i < this.state.originalInputContents.length;
            i++
          ) {
        ...
        if (!batches[batches.length - 1]._delegate._committed)
          promises.push(batches[batches.length - 1].commit().catch(() => null));
    ...
    render() {
      return this.state.setInaccessible ? (
        <Error404 />
      ) : (
        <div>
          <Prompt
                ...
                .map((contents, index) => (
                  <div className="create-set-input-row" key={index}>
                    <input
                      ...
                      value={
                        this.state.inputContents[index]
                          ? this.state.inputContents[index].term
                          : ""
                      }
                      autoComplete="off"
