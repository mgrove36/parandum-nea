...
describe("Parandum Firestore database", () => {
  ...
  it("Can update current user's user collection", async () => {
    ...
    await admin
      .collection("users")
      .doc(myId)
      .set({ theme: "default", coloredEdges: false });

    const db = getFirestore(myAuth);
    const myTestDoc = db.collection("users").doc(myId);
    await firebase.assertSucceeds(
      myTestDoc.update({ theme: "pink", coloredEdges: true })
    );
  });
  ...
  it("Can update current user's group role to owner when admin", async () => {
    ...
    await admin
      .collection("groups")
      .doc(groupOne)
      .set({ owners: [theirId], members: [myId] });
  ...
  it("Can update group display name when group owner", async () => {
    ...
    const testDoc = db.collection("groups").doc(groupOne);
    await firebase.assertSucceeds(
      testDoc.update({ display_name: "Test Group 1" })
    );
  });
  ...
  it("Can't delete current user's sets when they are in groups", async () => {
    ...
    await admin
      .collection("sets")
      .doc(setOne)
      .set({ owner: myId, groups: [groupOne] });
  ...
  it("Can create sets with current user as owner", async () => {
    ...
    const testDoc = db.collection("sets").doc(setOne);
    await firebase.assertSucceeds(
      testDoc.set({ owner: myId, public: true, title: "Set Title", groups: [] })
    );
  });
  ...
  it("Can't create sets with other user as owner", async () => {
    ...
    const testDoc = db.collection("sets").doc(setOne);
    await firebase.assertFails(
      testDoc.set({ owner: theirId, public: true, title: "Set Title" })
    );
  });
  ...
  it("Can't create sets with invalid data types", async () => {
    ...
    const testDoc = db.collection("sets").doc(setOne);
    await firebase.assertFails(
      testDoc.set({ owner: myId, public: 0, title: 0 })
    );
  });
  ...
  it("Can update set titles and visibility when set not in any groups", async () => {
    ...
    await admin
      .collection("sets")
      .doc(setOne)
      .set({ owner: myId, public: true, title: "Set Title", groups: [] });

    const db = getFirestore(myAuth);
    const testDoc = db.collection("sets").doc(setOne);
    await firebase.assertSucceeds(
      testDoc.update({ public: false, title: "Set Title 1" })
    );
  });
  ...
  it("Can't update set owners", async () => {
    ...
    await admin
      .collection("sets")
      .doc(setOne)
      .set({ owner: myId, public: true, title: "Set Title" });
  ...
  it("Can't update other users' sets", async () => {
    ...
    await admin
      .collection("sets")
      .doc(setOne)
      .set({ owner: theirId, public: true, title: "Set Title" });
  ...
  it("Can create vocab in sets with current user as owner", async () => {
    ...
    const testDoc = db
      ...
      .doc(vocabOne);
    await firebase.assertSucceeds(
      testDoc.set({ term: "value", definition: "value" })
    );
  });
  ...
  it("Can update sets' vocab", async () => {
    ...
    await admin
      ...
      .collection("vocab")
      .doc(vocabOne)
      .set({ term: "value", definition: "value" });
    ...
    const testDoc = db
      ...
      .doc(vocabOne);
    await firebase.assertSucceeds(
      testDoc.update({ term: "value 1", definition: "definition 1" })
    );
  });
  ...
  it("Can delete current user's progress data when not complete", async () => {
    ...
    await admin
      .collection("progress")
      .doc(progressOne)
      .set({ uid: myId, progress: 0, questions: [0] });
  ...
  it("Can't delete other users' progress data", async () => {
    ...
    await admin
      .collection("progress")
      .doc(progressOne)
      .set({ uid: theirId, progress: 0, questions: [0] });
  ...
  it("Can't delete current user's progress data when complete", async () => {
    ...
    await admin
      .collection("progress")
      .doc(progressOne)
      .set({ uid: theirId, progress: 1, questions: [0] });
  ...
  it("Can read current user's progress terms when language is not switched", async () => {
    ...
    await admin
      .collection("progress")
      .doc(progressOne)
      .set({ uid: myId, switch_language: false });
  ...
  it("Can't read other users' progress terms", async () => {
    ...
    await admin
      .collection("progress")
      .doc(progressOne)
      .set({ uid: theirId, switch_language: false });
  ...
  it("Can't read current user's progress terms when language is switched", async () => {
    ...
    await admin
      .collection("progress")
      .doc(progressOne)
      .set({ uid: myId, switch_language: true });
  ...
  it("Can't read other users' progress definitions", async () => {
    ...
    await admin
      .collection("progress")
      .doc(progressOne)
      .set({ uid: theirId, switch_language: true });
  ...
  it("Can read current user's progress terms and definitions when complete", async () => {
    ...
    await admin
      .collection("progress")
      .doc(progressOne)
      .set({ uid: myId, questions: ["q"], progress: 1 });
