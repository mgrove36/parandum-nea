  ...
  return db
    ...
    .then((groupDoc) => {
      if (!groupDoc.data() || !groupDoc.data().users) {
        throw new functions.https.HttpsError(
          "failed-precondition",
          "Group just created so users can't yet be retrieved - the only user is the group creator"
        );
      }
      ...
      if (groupUsers[uid] !== "owner") {
        throw new functions.https.HttpsError(
          "permission-denied",
          "You must be a group owner to retrieve group members' data"
        );
      }
      ...
      return Promise.all(
        Object.keys(groupUsers).map((userId) => {
          return admin
            .auth()
            .getUser(userId)
            .then((userRecord) => {
...
exports.createProgress = functions.https.onCall((data, context) => {
  ...
  if (typeof data.switch_language !== "boolean") {
    throw new functions.https.HttpsError(
      "invalid-argument",
      "switch_language must be a boolean"
    );
  }
  ...
  if (data.mode !== "questions" && data.mode !== "lives") {
    throw new functions.https.HttpsError(
      "invalid-argument",
      'mode must be "questions" or "lives"'
    );
  }
  ...
  return db
    .runTransaction(async (transaction) => {
        ...
        data.sets.map((setId) => {
          return transaction.get(setsId.doc(setId)).then((setDoc) => {
            ...
            if (!setDoc.data().public && setDoc.data().owner !== uid) {
              throw new functions.https.HttpsError(
                "permission-denied",
                "Insufficient permissions to access set"
              );
            }
            ...
            return transaction.get(setVocabCollectionId).then((setVocab) => {
              if (setVocab.docs.length < 1) {
                throw new functions.https.HttpsError(
                  "failed-precondition",
                  "Set must have at least one term/definition pair"
                );
              }
      ...
      const progressDocId = db.collection("progress").doc();

      const allSetTitles = [...Object.values(setTitlesDict)].sort();
      const setTitle =
        allSetTitles.slice(0, -1).join(", ") +
        (allSetTitles.length > 1 ? " & " : "") +
        allSetTitles.slice(-1);
    ...
    .then(async (data) => {
      ...
      shuffleArray(data.allVocab).forEach(async (doc, index, array) => {
        ...
        if (
          (data.mode == "questions" && index >= data.limit - 1) ||
      ...
      batches[batches.length - 1].set(data.progressDocId, data.dataToSet);
 ...
 * @return {string} The ID of the created progress document.
 */
exports.createProgressWithIncorrect = functions.https.onCall(
  (data, context) => {
    const uid = LOCAL_TESTING
      ? "M3JPrFRH6Fdo8XMUbF0l2zVZUCH3"
      : context.auth.uid;
      ...
      return transaction
        ...
        .then(async (doc) => {
          ...
          if (doc.data().uid !== uid)
            throw new functions.https.HttpsError(
              "permission-denied",
              "Can't use other users' progress records"
            );
          if (doc.data().incorrect.length < 1)
            throw new functions.https.HttpsError(
              "failed-precondition",
              "Progress record must have at least one incorrect answer"
            );
          ...
          dataToSet.questions.map(async (vocabId, index) => {
            ...
            promises.push(
              transaction
                .get(oldProgressDocId.collection("terms").doc(vocabId))
                .then((termDoc) => {
            ...
            promises.push(
              transaction
                .get(oldProgressDocId.collection("definitions").doc(vocabId))
                .then((termDoc) => {
                  return batches[currentBatchIndex].set(
          ...
          batches[batches.length - 1].set(newProgressDocId, dataToSet);
        ...
        .catch((error) => {
          throw new functions.https.HttpsError(
            "unknown",
            "Can't create new progress record from existing one"
          );
        });
    });
  }
 ...
 * @return {string} averagePercentage The average percentage mark for the current
 *          collection of sets. Only returned when the test is complete.
 * @return {boolean} correct Whether the provided answer was correct.
 * @return {array} correctAnswers An array of correct answers for the question just
 *          answered. If not all correct answers have yet been given, and the
 *          current answer is correct, this only contains the correct answers given
 *          so far.
 * @return {string} currentVocabId The vocab ID of the vocab item currently being
 *          evaluated.
 * @return {integer} duration The time taken for the test to be completed. Only
 *          returned when the test is complete.
 * @return {array} incorrectAnswers The vocab IDs of all incorrect answers given
 *          (including repeats for multiple incorrect answers). Only returned when
 *          the test is complete.
 * @return {integer} lives The total number of lives remaining. Only returned if mode
 *          is "lives".
 * @return {boolean} moreAnswers Whether or not there are more answers required for
 *          the current prompt.
 * @return {object} nextPrompt Details of the next prompt, if relevant. Null if last 
 *          question has been answered.
 * @return {string} nextPrompt.item The term/definition prompt for the next question.
 * @return {boolean} nextPrompt.set_owner User ID of the owner of the set file
 *          associated with the next prompt.
 * @return {integer} progress Total number of questions answered so far.
 * @return {integer} totalQuestions Total number of questions in the set (including
 *          duplicates after incorrect answers).
 ...
 *          (using Levenshtein distance or by detecting a null answer).
 */
  ...
  return db.runTransaction((transaction) => {
    return transaction.get(progressDocId).then((progressDoc) => {
      if (!progressDoc.exists) {
        throw new functions.https.HttpsError(
          "not-found",
          "Progress record " + progressId + " doesn't exist"
        );
      }
      if (uid !== progressDoc.data().uid) {
        throw new functions.https.HttpsError(
          "permission-denied",
          "Wrong user's progress"
        );
      }
      if (
        progressDoc.data().progress >= progressDoc.data().questions.length ||
        (progressDoc.data().mode === "lives" && progressDoc.data().lives <= 0)
      ) {
        throw new functions.https.HttpsError(
          "permission-denied",
      ...
      termDocId = progressDocId.collection("terms").doc(currentVocab);
      definitionDocId = progressDocId
        .collection("definitions")
        .doc(currentVocab);

      return transaction
        .get(progressDoc.data().switch_language ? termDocId : definitionDocId)
        .then((answerDoc) => {
          ...
          const cleansedSplitCorrectAnswers = splitCorrectAnswers.map(
            (answer) => cleanseVocabString(answer)
          );
          ...
          if (isCorrectAnswer) {
            ...
            if (!prevCorrect) {
              docData.current_correct = [
                splitCorrectAnswers[correctAnswerIndex],
              ];
            } else if (
              !prevCorrect.includes(splitCorrectAnswers[correctAnswerIndex])
            ) {
              docData.current_correct.push(
                splitCorrectAnswers[correctAnswerIndex]
              );
            }
            ...
            docData.current_correct = [];
            docData.questions = doneQuestions.concat(
              shuffleArray(notDoneQuestions)
            );
            returnData.totalQuestions = docData.questions.length;
            returnData.totalIncorrect = docData.incorrect.length;

            userGroups = transaction
              .get(db.collection("users").doc(uid).collection("groups"))
              .then((querySnapshot) => querySnapshot.docs.map((doc) => doc.id));
            incorrectAnswerDoc = db.collection("incorrect_answers").doc();
            prompt = transaction
              .get(
                progressDoc.data().switch_language ? definitionDocId : termDocId
              )
              .then((doc) => doc.data().item);
          }

          if (!returnData.moreAnswers) {
            if (
              docData.progress >= docData.questions.length ||
              (mode === "lives" && docData.lives <= 0)
            ) {
              ...
              if (mode === "lives" && docData.lives <= 0)
                docData.questions.length = returnData.totalQuestions =
                  docData.progress;

              const completedProgressDocId = db
                .collection("completed_progress")
                .doc(progressDoc.data().setIds.sort().join("__"));
              return transaction
                .get(completedProgressDocId)
                .then(async (completedProgressDoc) => {
                  if (!completedProgressDoc.exists)
                    throw new Error("Completed progress doc doesn't exist");

                  if (!isCorrectAnswer)
                    transaction.set(incorrectAnswerDoc, {
                      uid: uid,
                      groups: await userGroups,
                      term: progressDoc.data().switch_language
                        ? correctAnswers
                        : await prompt,
                      definition: progressDoc.data().switch_language
                        ? await prompt
                        : correctAnswers,
                  ...
                  const totalPercentage =
                    completedProgressDoc.data().total_percentage +
                    (docData.correct.length / docData.questions.length) * 100;
                  ...
                  returnData.averagePercentage = (
                    totalPercentage / attempts
                  ).toFixed(2);
                  transaction.set(progressDocId, docData);
                  return returnData;
                })
...
exports.setAdmin = functions.https.onCall(async (data, context) => {
  ...
  if (isAdmin) {
    if (uid !== targetUser) {
      ...
      throw new functions.https.HttpsError(
        "permission-denied",
        "Cannot change admin status of authenticated user"
      );
    }
  } else {
    throw new functions.https.HttpsError(
      "permission-denied",
      "Must be an admin to change other users' admin states"
    );
  }
});
  ...
  return db.runTransaction((transaction) => {
    return transaction.get(setDocId).then((setDoc) => {
      return transaction.get(userGroupDocId).then((userGroupDoc) => {
        ...
        if (
          auth &&
          (setDoc.data().public || setDoc.data().owner == uid) &&
          (userRole == "contributor" || userRole == "owner" || isAdmin)
        ) {
          let setDocData = setDoc.data();
          if (
            setDocData.groups != null &&
            setDocData.groups.includes(groupId)
          ) {
            throw new functions.https.HttpsError(
              "permission-denied",
              "Set is already part of group"
            );
          } else {
            return transaction.get(groupDocId).then((groupDoc) => {
              ...
              return Promise.all([
                transaction.set(setDocId, setDocData),
                transaction.set(groupDocId, groupDocData),
              ]).then(() => true);
            });
          }
        } else {
          throw new functions.https.HttpsError(
            "permission-denied",
            "Insufficient permisisons to add set to group"
          );
        }
      });
    });
      ...
      return transaction.get(userGroupDocId).then((userGroupDoc) => {
        ...
        if (auth && (userRole == "owner" || isAdmin)) {
          ...
          if (
            ...
            throw new functions.https.HttpsError(
              "permission-denied",
              "Set is not part of group"
            );
          } else {
            return transaction.get(groupDocId).then((groupDoc) => {
              setDocData.groups = setDocData.groups.filter(
                (item) => item !== groupId
              );
              let groupDocData = groupDoc.data();
              groupDocData.sets = groupDocData.sets.filter(
                (item) => item !== setId
              );
          ...
          throw new functions.https.HttpsError(
            "permission-denied",
            "Insufficient permisisons to remove set from group"
          );
        }
      });
    });
  ...
  await db.runTransaction((transaction) => {
    ...
    return transaction.get(groupDocId).then((groupDoc) => {
      ...
      if (
        ...
        typeof snap.data() !== "undefined" &&
        typeof snap.data().role !== "undefined"
      ) {
        groupData.users[context.params.userId] = snap.data().role;
      } else {
        delete groupData.users[context.params.userId];
      }
      return transaction.set(groupDocId, groupData);
    });
  });
 ...
 * @return {boolean} Returns true on completion.
 */
exports.userGroupRoleCreated = functions.firestore
 ...
 * @return {boolean} Returns true on completion.
 */
exports.userGroupRoleUpdated = functions.firestore
  .document("users/{userId}/groups/{groupId}")
    ...
    await Promise.all([
      ...
      Promise.all(
        sets.map((setId) => {
          return db.runTransaction((transaction) => {
            return transaction
              .get(db.collection("sets").doc(setId))
              .then((setDoc) => {
                ...
                if (!data.groups) {
                  ...
                  data.groups.splice(
                    data.groups.indexOf(context.params.groupId),
                    1
                  );
                }
                transaction.set(db.collection("sets").doc(setId), data);
              });
          });
        })
      ),
 ...
 */
exports.setDeleted = functions.firestore
  .document("sets/{setId}")
  .onDelete(async (snap, context) => {
    await deleteCollection(db, "/sets/" + context.params.setId + "/vocab", 500);
 ...
 * @return {boolean} Returns true on completion.
 */
exports.progressDeleted = functions.firestore
